...
$staging  = "C:\maolan-staging\editor"
...
Write-Host "Building maolan-editor (release)..."
...
Copy-Item (Join-Path $sourceDir "target\$target\release\maolan-editor.exe") $staging -Force
...
$nsiTemp = "$env:TEMP\maolan-editor-installer"
...
$projectIcon = Join-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) "maolan\assets\images\maolan-icon.ico"
...
$nsisDefines = @(
    "/INPUTCHARSET", "UTF8",
    "/DMAOLAN_EDITOR_VERSION=$pkgVersion",
    "/DMAOLAN_EDITOR_PRODUCT_VERSION=$productVersion"
)
if ($iconPath) {
    $nsisDefines += "/DMAOLAN_EDITOR_ICON=$iconPath"
}
...
$outFile = "maolan-editor-$pkgVersion.windows.amd64.exe"
Copy-Item "$nsiTemp\maolan-editor-setup.exe" "$distDir\$outFile" -Force -ErrorAction SilentlyContinue
